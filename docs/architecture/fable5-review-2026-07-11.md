...
2. **Credenciais em texto claro no banco** — `AIProvider.apiKey` (`schema.prisma:113`) e `DataSource.config` ("JSON string with credentials/settings", `schema.prisma:125`) persistem segredos sem cifragem no `dev.db`. Combinado com o banco dentro do OneDrive, os segredos sobem para a nuvem da Microsoft. Sugiro achado novo, severidade alta, com remediação via `safeStorage` do Electron ou keyring do SO (Fase 1). *(Nota 2026-07-14: com o projeto fora do OneDrive, o vetor de upload para a nuvem deixou de existir.)* **Resolvido em 2026-07-14:** auditoria confirmou que `AIProvider`/`DataSource` eram modelos sem nenhum uso no código e com tabelas vazias (credenciais LLM já vivem só em env vars server-side desde o item 7 da Fase 0). Em vez de cifrar colunas mortas, os modelos foram removidos do schema com migração `20260714184514_drop_plaintext_secret_models` — o banco não tem mais onde guardar segredo em claro. Se persistência de segredos for necessária no futuro, usar `safeStorage`/keyring, nunca coluna em claro.
