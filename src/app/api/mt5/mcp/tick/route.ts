/**
 * Último tick de um símbolo (read-only) — MT5 MCP nativo — WR Trading Pro
 *
 * GET ?symbol=XXXX: consulta a capability `tick` via MCP nativo e devolve o
 * tick sanitizado. Somente leitura — nenhuma rota sob /api/mt5/mcp/** envia,
 * modifica ou fecha ordem/posição.
 *
 * Diferente de /positions, `symbol` é obrigatório aqui — um tick sem símbolo
 * não tem sentido (não existe "todos os ticks"). Ausência retorna 400, sem
 * sequer consultar o MCP nativo.
 *
...
import { getTick } from '@/lib/server/mt5-mcp-tools';
