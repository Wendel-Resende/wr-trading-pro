/**
 * Book de ofertas / market depth (read-only) — MT5 MCP nativo — WR Trading Pro
 *
 * GET ?symbol=XXXX: consulta a capability `market_book` via MCP nativo e
 * devolve o objeto sanitizado ({bids,asks} ou o shape cru, dependendo do
 * que o servidor nativo devolver). Somente leitura — nenhuma rota sob
 * /api/mt5/mcp/** envia, modifica ou fecha ordem/posição.
 *
 * `symbol` é obrigatório (como em /tick) — book sem símbolo não tem
...
import { getMarketBook } from '@/lib/server/mt5-mcp-tools';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, data: { symbol, book } });
  } catch (error) {
    const payload: Mt5McpErrorPayload =
      error instanceof Mt5McpError
        ? error.toPayload()
        : { code: 'MT5_MCP_TOOL_ERROR', message: MT5_MCP_ERROR_MESSAGES.MT5_MCP_TOOL_ERROR };
    console.error('[api/mt5/mcp/order-book]', payload.code);
