...
/** Validates and parses an instant at the adapter boundary; throws InvalidAgentRunInputError otherwise (fail closed). */
export const requireAdapterInstant = (value: string, field: string): Instant => {
  const instant = parseInstant(value);
  if (!instant) {
    throw new InvalidAgentRunInputError(`${field} inválido: timestamp ISO-8601 exigido`, [field]);
  }
  if (!hasAtMostMillisecondPrecision(value)) {
    throw new InvalidAgentRunInputError(
