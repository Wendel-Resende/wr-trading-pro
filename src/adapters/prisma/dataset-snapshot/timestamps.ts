 ...
 * adapter's boundary is required to have at most 3 fraction digits.
 */
...
/** Validates and parses an instant at the adapter boundary; throws InvalidDatasetSnapshotInputError otherwise (fail closed). */
export const requireAdapterInstant = (value: string, field: string): Instant => {
  const instant = parseInstant(value);
  if (!instant) {
    throw new InvalidDatasetSnapshotInputError(`${field} inválido: timestamp ISO-8601 exigido`, [field]);
  }
  if (!hasAtMostMillisecondPrecision(value)) {
    throw new InvalidDatasetSnapshotInputError(
