...
try {
  ...
  run(process.execPath, ['node_modules/typescript/bin/tsc', '-p', 'scripts/market-bar/tsconfig.json']);
  ...
  tempDir = mkdtempSync(join(os.tmpdir(), 'wr-market-bar-test-'));
  ...
  run(process.execPath, ['scripts/market-bar/.dist/scripts/market-bar/market-bar-test.js'], testEnv);
} catch (error) {
