...
try {
  ...
  run(process.execPath, ['node_modules/typescript/bin/tsc', '-p', 'scripts/mcp/tsconfig.json']);
  ...
  // below throws.
  tempDir = mkdtempSync(join(os.tmpdir(), 'wr-mcp-test-'));
  ...
  run(process.execPath, ['scripts/mcp/.dist/scripts/mcp/mcp-test.js'], testEnv);
} catch (error) {
