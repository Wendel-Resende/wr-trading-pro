...
try {
  ...
  run(process.execPath, ['node_modules/typescript/bin/tsc', '-p', 'scripts/cvm-facts/tsconfig.json']);
  ...
  tempDir = mkdtempSync(join(os.tmpdir(), 'wr-cvm-facts-test-'));
  ...
  run(process.execPath, ['scripts/cvm-facts/.dist/scripts/cvm-facts/cvm-facts-test.js'], testEnv);
} catch (error) {
