...
try {
  ...
  run(process.execPath, ['node_modules/typescript/bin/tsc', '-p', 'scripts/risk-policy/tsconfig.json']);
  ...
  // below throws.
  tempDir = mkdtempSync(join(os.tmpdir(), 'wr-risk-policy-test-'));
  ...
  run(process.execPath, ['scripts/risk-policy/.dist/scripts/risk-policy/risk-policy-test.js'], testEnv);
} catch (error) {
