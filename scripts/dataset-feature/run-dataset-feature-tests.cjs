...
try {
  ...
  run(process.execPath, ['node_modules/typescript/bin/tsc', '-p', 'scripts/dataset-feature/tsconfig.json']);
  ...
  // below throws.
  tempDir = mkdtempSync(join(os.tmpdir(), 'wr-dataset-feature-test-'));
  ...
  run(process.execPath, ['scripts/dataset-feature/.dist/scripts/dataset-feature/dataset-feature-test.js'], testEnv);
} catch (error) {
