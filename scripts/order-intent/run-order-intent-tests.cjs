...
try {
  ...
  run(process.execPath, ['node_modules/typescript/bin/tsc', '-p', 'scripts/order-intent/tsconfig.json']);
  ...
  // below throws.
  tempDir = mkdtempSync(join(os.tmpdir(), 'wr-order-intent-test-'));
  ...
  run(process.execPath, ['scripts/order-intent/.dist/scripts/order-intent/order-intent-test.js'], testEnv);
} catch (error) {
