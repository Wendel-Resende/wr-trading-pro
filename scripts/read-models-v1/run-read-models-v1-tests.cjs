...
try {
  ...
  run(process.execPath, ['node_modules/typescript/bin/tsc', '-p', 'scripts/read-models-v1/tsconfig.json']);

  // Deterministic, disposable SQLite database outside the repository; never
  // touches prisma/dev.db. Removed in `finally` even if a step below throws.
  tempDir = mkdtempSync(join(os.tmpdir(), 'wr-read-models-v1-test-'));
  ...
  run(process.execPath, ['scripts/read-models-v1/.dist/scripts/read-models-v1/read-models-v1-test.js'], testEnv);
} catch (error) {
