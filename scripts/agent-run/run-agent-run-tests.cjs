...
try {
  ...
  run(process.execPath, ['node_modules/typescript/bin/tsc', '-p', 'scripts/agent-run/tsconfig.json']);
  ...
  // below throws.
  tempDir = mkdtempSync(join(os.tmpdir(), 'wr-agent-run-test-'));
  ...
  run(process.execPath, ['scripts/agent-run/.dist/scripts/agent-run/agent-run-test.js'], testEnv);
} catch (error) {
