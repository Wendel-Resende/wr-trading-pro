...
try {
  ...
  run(process.execPath, ['node_modules/typescript/bin/tsc', '-p', 'scripts/model-version/tsconfig.json']);

  tempDir = mkdtempSync(join(os.tmpdir(), 'wr-model-version-test-'));
  ...
  run(process.execPath, ['scripts/model-version/.dist/scripts/model-version/model-version-test.js'], testEnv);
} catch (error) {
