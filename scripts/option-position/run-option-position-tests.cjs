...
try {
  ...
  run(process.execPath, ['node_modules/typescript/bin/tsc', '-p', 'scripts/option-position/tsconfig.json']);

  tempDir = mkdtempSync(join(os.tmpdir(), 'wr-option-position-test-'));
  ...
  run(process.execPath, ['scripts/option-position/.dist/scripts/option-position/option-position-test.js'], testEnv);
} catch (error) {
