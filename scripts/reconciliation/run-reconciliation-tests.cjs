...
try {
  ...
  run(process.execPath, ['node_modules/typescript/bin/tsc', '-p', 'scripts/reconciliation/tsconfig.json']);
  ...
  // below throws.
  tempDir = mkdtempSync(join(os.tmpdir(), 'wr-reconciliation-test-'));
  ...
  run(process.execPath, ['scripts/reconciliation/.dist/scripts/reconciliation/reconciliation-test.js'], testEnv);
} catch (error) {
