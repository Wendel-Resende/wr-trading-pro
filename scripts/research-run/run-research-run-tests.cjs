...
try {
  ...
  run(process.execPath, ['node_modules/typescript/bin/tsc', '-p', 'scripts/research-run/tsconfig.json']);

  tempDir = mkdtempSync(join(os.tmpdir(), 'wr-research-run-test-'));
  ...
  run(process.execPath, ['scripts/research-run/.dist/scripts/research-run/research-run-test.js'], testEnv);
} catch (error) {
