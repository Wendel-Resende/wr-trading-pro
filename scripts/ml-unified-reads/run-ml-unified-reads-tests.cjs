...
try {
  ...
  run(process.execPath, ['node_modules/typescript/bin/tsc', '-p', 'scripts/ml-unified-reads/tsconfig.json']);

  tempDir = mkdtempSync(join(os.tmpdir(), 'wr-ml-unified-reads-test-'));
  ...
  run(process.execPath, ['scripts/ml-unified-reads/.dist/scripts/ml-unified-reads/ml-unified-reads-test.js'], testEnv);
} catch (error) {
